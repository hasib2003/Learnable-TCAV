...
class DefaultClassifier(Classifier):
    ...
    def train_and_eval(
        ...
        **kwargs: Any,
        ...
        # print(f"found labels {uni=}")
        # print(f"found freq {count=}")
        ...
        predict = self.lm.classes()[torch.argmax(predict, dim=1).to("cpu")]  # type: ignore
        score = predict.long() == y_test.long().cpu()

        accs = score.float().mean()

        # print(accs)
        # print(f"{self.lm.classes().detach().numpy()=}")
    ...
    def weights(self) -> Tensor:
        ...
        if weights.shape[0] == 1:
            ...
            weights = torch.stack([-1 * weights[0], weights[0]]).to("cpu")
    ...
    def classes(self) -> List[int]:
        ...
        classes = self.lm.classes().detach().numpy()
