...
def train_n_batch_alignment(model, loader, criterion, optimizer, device,alpha=0.5,n=20):

    """
    trains the model on alignemnt task only for batches (randomized if loader is suffled)
    """
    ...
    model = model.to(device)
    concept_optimizer = optim.Adam(model.parameters(), lr=optimizer.param_groups[0]['lr']) 

    model.train()

    total_loss = 0
        
    tq = tqdm(loader, desc="Training (n_batch_alignment)")
    
    for idx,(images, labels) in enumerate(tq):
